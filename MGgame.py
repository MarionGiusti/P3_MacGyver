...
while (needle.x, needle.y) == (0, 0) or \
	(needle.x, needle.y) == ((NBR_SPRITE_SIDE - 1) * SPRITE_SIZE, \
		(NBR_SPRITE_SIDE - 1) * SPRITE_SIZE) or \
	(ether.x, ether.y) == (0, 0) or \
	(ether.x, ether.y) == ((NBR_SPRITE_SIDE - 1) * SPRITE_SIZE, \
		(NBR_SPRITE_SIDE - 1) * SPRITE_SIZE) or \
	(tube.x, tube.y) == (0, 0) or \
	(tube.x, tube.y) == ((NBR_SPRITE_SIDE - 1) * SPRITE_SIZE, \
		(NBR_SPRITE_SIDE - 1) * SPRITE_SIZE) or \
...
while continue_main:
	...
	while continue_game:
		...
		if (mg.x, mg.y) == (needle.x, needle.y):
			(needle.x, needle.y) = ((len(level.structure) - 1) * SPRITE_SIZE, \
				(len(level.structure) - 1) * SPRITE_SIZE)
			count_survival += 1
		elif (mg.x, mg.y) == (ether.x, ether.y):
			(ether.x, ether.y) = ((len(level.structure) - 1) * SPRITE_SIZE, \
				(len(level.structure) - 1) * SPRITE_SIZE)
			count_survival += 1
		elif (mg.x, mg.y) == (tube.x, tube.y):
			(tube.x, tube.y) = ((len(level.structure) - 1) * SPRITE_SIZE, \
			(len(level.structure) - 1) * SPRITE_SIZE)
			count_survival += 1
